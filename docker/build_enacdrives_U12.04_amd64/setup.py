...
class bdist_rpm(distutils.command.bdist_rpm.bdist_rpm):
    ...
    def run(self):
        ...
        specFile = os.path.join(
            self.rpm_base, "SPECS", "%s.spec" % self.distribution.get_name()
        )
        ...
        self.move_file(
            os.path.join("dist", origFileName), os.path.join("dist", newFileName)
        )
...
class build_ext(distutils.command.build_ext.build_ext):
    def build_extension(self, ext):
        ...
        if sys.platform == "win32":
            ...
            if not vars.get("Py_ENABLE_SHARED", 0):
                ...
                # print("Append python%s" % vars["LDVERSION"])
    ...
    def get_ext_filename(self, name):
        fileName = distutils.command.build_ext.build_ext.get_ext_filename(self, name)
...
if sys.platform == "win32":
    ...
    libraries = []
utilModule = Extension("cx_Freeze.util", ["source/util.c"], libraries=libraries)
...
consoleKeepPath = Extension(
    "cx_Freeze.bases.ConsoleKeepPath",
