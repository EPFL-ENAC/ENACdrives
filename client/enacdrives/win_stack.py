...
def os_check(ui):
    ...
    """
    if (
        CONST.OS_DISTRIB == "Microsoft"
        and CONST.OS_SYS == "Windows"
        and CONST.OS_VERSION == "XP"
    ):
        ui.notify_user(
            "We're sorry but ENACdrives is not supported on Windows XP. See <a href='{}'>full documentation</a>.".format(
                CONST.DOC_URL
            )
        )
...
def cifs_mount(mount):
    ...
    try:
        ...
        if e.winerror == 5:  # (5, 'WNetAddConnection2', 'Access is denied.')
            ...
            e.winerror == 86
        ):  # (86, "WNetAddConnection2", "The specified network password is not correct.")
            pass
        elif (
            e.winerror == 1326
        ):  # (1326, "WNetAddConnection2", "Logon failure: unknown user name or bad password.")
            pass
        elif (
            e.winerror == 31
        ):  # (31, 'WNetAddConnection2', 'A device attached to the system is not functioning.')
            pass
        elif (
            e.winerror == 53
        ):  # (53, 'WNetAddConnection2', 'The network path was not found.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 55
        ):  # (55, 'WNetAddConnection2', 'The specified network resource or device is no longer available.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 64
        ):  # (64, 'WNetAddConnection2', 'Le nom réseau spécifié n’est plus disponible.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 67
        ):  # (67, 'WNetAddConnection2', 'The network name cannot be found.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 71
        ):  # (71, 'WNetAddConnection2', 'No more connections can be made to this remote computer at this time because there are already as many connections as the computer can accept.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 85
        ):  # (85, 'WNetAddConnection2', 'The local device name is already in use.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 121
        ):  # (121, 'WNetAddConnection2', 'The semaphore timeout period has expired.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 1202
        ):  # (1202, 'WNetAddConnection2', 'The local device name has a remembered connection to another network resource.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 1208
        ):  # (1208, 'WNetAddConnection2', 'An extended error has occurred.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 1219
        ):  # (1219, 'WNetAddConnection2', 'Multiple connections to a server or shared resource by the same user, using more than one user name, are not allowed. Disconnect all previous connections to the server or shared resource and try again.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 1222
        ):  # (1222, 'WNetAddConnection2', 'The network is not present or not started.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 1265
        ):  # (1265, 'WNetAddConnection2', 'The system detected a possible attempt to compromise security. Please ensure that you can contact the server that authenticated you.')
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 1272
        ):  # (1272, 'WNetAddConnection2', "You can't access this shared folder because your organization's security policies block unauthenticated guest access. These policies help protect your PC from unsafe or malicious devices on the network.")
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 1311
        ):  # (1311, 'WNetAddConnection2', 'There are currently no logon servers available to service the logon request.')
            ...
            e.winerror == 1331
        ):  # (1331, 'WNetAddConnection2', "This user can't sign in because this account is currently disabled.")
            mount.ui.notify_user(e.strerror)
            return False
        elif (
            e.winerror == 1907
        ):  # (1907, 'WNetAddConnection2', "The user's password must be changed before signing in.")
            pass
        else:
    ...
    for _ in range(3):
        try:
            ...
            win32wnet.WNetAddConnection2(
                ...
                None,
                r"{0}\{1}".format(
                    mount.settings["realm_domain"], mount.settings["realm_username"]
                ),
                pw,
                0,
            )
...
def cifs_umount(mount):
    try:
        ...
        if (
            e.winerror == 2401
        ):  # (2401, "WNetCancelConnection2", "There are open files on the connection.")
            mount.ui.notify_user(e.strerror)
        elif (
            e.winerror == 2250
        ):  # (2250, 'WNetCancelConnection2', 'This network connection does not exist.')
            mount.ui.notify_user(e.strerror)
        else:
            Output.error("failed : {0}".format(e))
            debug_send("umount:\n{0}".format(e))
