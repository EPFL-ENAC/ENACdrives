...
def get_config():
    ...
    try:
        ...
        if username is not None:
            Output.verbose(
                "Loaded username '{}' from User context. ({})".format(
                    username, CONST.USER_CONF_FILE
                )
            )
        else:
            Output.normal(
                "Username not found in User context. ({})".format(CONST.USER_CONF_FILE)
            )
    ...
    if username is None:
        if CONST.AD_USERNAME is not None:
            ...
            Output.verbose(
                "Loaded username '{}' from environment variables (user in domain '{}').".format(
                    username, CONST.AD_DOMAIN
                )
            )
            ...
            Output.normal(
                "Username not found in environment variables (user not in a domain)."
            )
    ...
    if username is not None:
        ...
        cache_filename = os.path.join(
            CONST.USER_CACHE_DIR, hashlib.md5(config_url.encode()).hexdigest()
        )
        try:
            with urllib.request.urlopen(
                config_url, timeout=CONST.URL_TIMEOUT
            ) as response:
            ...
            Output.verbose(
                "Loaded config from ENACdrives server ({})".format(config_url)
            )
        except (socket.timeout, urllib.error.URLError, ConfigException) as e:
            Output.warning(
                "Could not load config ENACdrives server. ({})".format(config_url)
            )
            ...
            try:
                ...
                Output.normal(
                    "Loaded config from cache file. ({})".format(cache_filename)
                )
            except FileNotFoundException:
                Output.warning(
                    "Could not load config from cache file. ({})".format(cache_filename)
                )
    else:
    ...
    try:
        ...
        Output.verbose(
            "Loaded config from System context. ({})".format(CONST.SYSTEM_CONF_FILE)
        )
    except FileNotFoundException:
        Output.normal(
            "No config found from System context. ({})".format(CONST.SYSTEM_CONF_FILE)
        )
    ...
    if user_config is not None:
        ...
        Output.verbose(
            "Loaded config from User context. ({})".format(CONST.USER_CONF_FILE)
        )
    else:
        Output.normal(
            "No config found from User context. ({})".format(CONST.USER_CONF_FILE)
        )
...
def save_username(username):
    ...
    try:
        ...
        if username_line_nb is not None:
            Output.verbose(
                "Changing to username='{}' in config file {}".format(
                    username, CONST.USER_CONF_FILE
                )
            )
        ...
        # username not found, but [global] found
            ...
            Output.verbose(
                "Saving username='{}' in config file {}".format(
                    username, CONST.USER_CONF_FILE
                )
            )
        ...
        Output.verbose(
            "Saving username='{}' to new config file {}".format(
                username, CONST.USER_CONF_FILE
            )
        )
        lines.insert(0, "[global]\n")
...
def save_bookmark(section_name, bookmark_on):
    ...
    try:
        ...
        if option_line_nb is not None:
            Output.verbose(
                "Changing {}'s bookmark='{}' in config file {}".format(
                    section_name, bookmark_on, CONST.USER_CONF_FILE
                )
            )
        ...
        # bookmark not found, but [CIFS_mount] found
            ...
            Output.verbose(
                "Saving {}'s bookmark='{}' in config file {}".format(
                    section_name, bookmark_on, CONST.USER_CONF_FILE
                )
            )
            lines.insert(
                section_name_line_nb + 1, "bookmark = {}\n".format(bookmark_on)
            )
        ...
        Output.verbose(
            "Saving {}'s bookmark='{}' to new config file {}".format(
                section_name, bookmark_on, CONST.USER_CONF_FILE
            )
        )
        lines.append("[CIFS_mount]\n")
...
def save_windows_letter(section_name, letter):
    ...
    try:
        ...
        if option_line_nb is not None:
            Output.verbose(
                "Changing {}'s Windows_letter='{}' in config file {}".format(
                    section_name, letter, CONST.USER_CONF_FILE
                )
            )
        ...
        # Windows_letter not found, but [CIFS_mount] found
            ...
            Output.verbose(
                "Saving {}'s Windows_letter='{}' in config file {}".format(
                    section_name, letter, CONST.USER_CONF_FILE
                )
            )
            lines.insert(
                section_name_line_nb + 1, "Windows_letter = {}\n".format(letter)
            )
        ...
        Output.verbose(
            "Saving {}'s Windows_letter='{}' to new config file {}".format(
                section_name, letter, CONST.USER_CONF_FILE
            )
        )
        lines.append("[CIFS_mount]\n")
    ...
    if option == "Linux_CIFS_method":
        if value not in ("gvfs", "mount.cifs"):
            raise ConfigException(
                "Error, Linux_CIFS_method has to be 'gvfs' or 'mount.cifs'."
            )
        ...
        if bool(re.search(r"[^a-zA-Z0-9.-]", value)):
            raise ConfigException(
                "Error, server_name can only contain alphanumeric, and '-' and '.' symbols."
            )
        ...
        if value != "":
            ...
            if not re.match(r"[A-Z]:$", value):
                raise ConfigException(
                    "Error, Windows drive letter has to be empty or a letter formated like 'Z:'."
                )
    elif option == "icon":
        if value not in ("none", "info", "warning", "critical"):
            raise ConfigException(
                "Error, inknown icon type '{}'. Should be one of none, info, warning, critical".format(
                    value
                )
            )
    return value
...
def read_config_source(src):
    ...
    def save_current_section():
        try:
            ...
            Output.error(
                "Expected name option not found at line {}. Skipping that section.".format(
                    section_line_nb
                )
            )
    ...
    for line in src.readlines():
        ...
        if l.startswith("["):
            try:
                ...
                continue
            if (
                current_section_name in multi_entries_sections
                and current_section_values != {}
            ):
            ...
            if new_section in allowed_options:
                ...
                Output.error(
                    "Unexpected section name '{}' at line {}:\n{}".format(
                        new_section, line_nb, line
                    )
                )
                current_section_name = ""
                nb_unexpected_lines += 1
    ...
    if nb_unexpected_lines > (line_nb / 2):
        Output.warning(
            "Found {}/{} ({}%) unexpected lines. Skipping this source.".format(
                nb_unexpected_lines, line_nb, nb_unexpected_lines * 100.0 / line_nb
            )
        )
        raise ConfigException("Too many unexpected lines found. Skipping this source.")
    elif nb_unexpected_lines > 0:
        Output.warning(
            "Found {}/{} ({}%) unexpected lines.".format(
                nb_unexpected_lines, line_nb, nb_unexpected_lines * 100.0 / line_nb
            )
        )
...
def validate_config(cfg):
    ...
    def expect_option(entry, section, option, alert=True, ored_options=None):
        if option not in entry:
            if alert:
                ...
                Output.error(
                    "expected '{}' option in {} section.".format(option, section)
                )
            return False
        return True
    ...
    for m_name in cfg.get("CIFS_mount", {}):
        ...
        if "unc" in cfg["CIFS_mount"][m_name]:
            ...
            if m:
                ...
                Output.error(
                    "unrecognized 'unc' option in CIFS_mount section ({}).".format(
                        cfg["CIFS_mount"][m_name]["unc"]
                    )
                )

        is_ok = (
            expect_option(cfg["CIFS_mount"][m_name], "CIFS_mount", "label")
            and expect_option(cfg["CIFS_mount"][m_name], "CIFS_mount", "server_name")
            and expect_option(cfg["CIFS_mount"][m_name], "CIFS_mount", "server_path")
            and expect_option(cfg["CIFS_mount"][m_name], "CIFS_mount", "local_path")
            and (
                expect_option(cfg.get("global", {}), "CIFS_mount", "realm", alert=False)
                or expect_option(cfg["CIFS_mount"][m_name], "CIFS_mount", "realm")
            )
        )
        if is_ok:
    ...
    for realm in expected_realms:
        if realm not in cfg.get("realm", []):
            ...
            for m_name in expected_realms[realm]:
                Output.error(
                    "Removing CIFS_mount '{}' depending on realm '{}'.".format(
                        m_name, realm
                    )
                )
                invalid_cifs_m.append(m_name)
    ...
    for realm in cfg.get("realm", []):
        is_ok = expect_option(
            cfg["realm"][realm], "realm", "username"
        ) and expect_option(cfg["realm"][realm], "realm", "domain")
        if not is_ok:
    ...
    for net in expected_networks_by_CIFS_m:
        if net not in cfg.get("network", []):
            ...
            for m_name in expected_networks_by_CIFS_m[net]:
                Output.error(
                    "Removing 'require_network' to CIFS_mount '{}'.".format(m_name)
                )
                del cfg["CIFS_mount"][m_name]["require_network"]

    for net in cfg.get("network", []):
        is_ok = (
            expect_option(cfg["network"][net], "network", "ping", alert=False)
            or expect_option(
                cfg["network"][net], "network", "cifs", ored_options=("ping", "cifs")
            )
        ) and expect_option(cfg["network"][net], "network", "error_msg")
        if not is_ok:
    ...
    while had_change:
        ...
        for net in cfg.get("network", []):
            ...
            if parent_network is None:
                continue
            if (
                parent_network in invalid_network
                or parent_network not in cfg["network"]
            ):
                invalid_network.append(net)
                had_change = True

    for msg in cfg.get("msg", []):
        is_ok = expect_option(cfg["msg"][msg], "msg", "text")
